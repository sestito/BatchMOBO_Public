...
class ZDT2(TestFunction):
    name = "ZDT2"
    '''
    x is a vector of values within [0,1]
    '''
    # TODO verify x is betwen 0 and 1
    ...
    def set_bounds(self) -> None:
        ...
        For the ZDT2 problem, the lower bound is 0 and the upper bound is 1 for all parameters
        '''
    ...
    def __h(self, x):
        return 1 - np.power(self.__f1(x) / self.__g(x), 2)
    ...
    def plot_data(self, number_of_data_points = 10000) -> np.ndarray:
        ...
        X = np.zeros((number_of_data_points, 2))
        X[:, 0] = np.linspace(0, 1, number_of_data_points)
